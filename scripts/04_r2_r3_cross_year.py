...
for ax,(k,(yte,pred,seen)) in zip(axes,scatter.items()):
    ax.scatter(yte[~seen],pred[~seen],s=14,alpha=.6,label="unseen",color="#4C72B0")
    ax.scatter(yte[seen],pred[seen],s=26,alpha=.9,label=f"seen (n={int(seen.sum())})",color="#C44E52",edgecolor="k",linewidth=.4)
...
fig.tight_layout(); fig.savefig(os.path.join(RES,"FIG_cross_year_scatter.png"),dpi=130)
