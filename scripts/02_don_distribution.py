...
for ax,k in zip(axes,["2021","2022","across"]):
    s=dfs[k]["DON"].astype(float)
    ax.hist(s,bins=25,color="#4C72B0",edgecolor="white")
    ax.axvline(1.0,color="#C44E52",ls="--",lw=1.1,label="FDA / EU current (1.0)")
    ax.axvline(1.25,color="#8172B3",ls=":",lw=1.1,label="EU former (1.25)")
    ax.axvline(s.median(),color="#55A868",ls="-",lw=1,label=f"median {s.median():.2f}")
...
fig.savefig(os.path.join(RES,"FIG_don_distribution.png"),dpi=130)
